...
def we(seq_i, seq_j, gap, n_alignments=5, threshold=0):
    ...
    used = used = [[False for _ in range(len(seq_j) + 1)] for _ in range(len(seq_i) + 1)]
    ...
    for _ in range(n_alignments): 
        ...
        while traceback[i][j] != 2:
            ...
            if traceback[i][j] == 0:  # Match/mismatch
                i -= 1
                j -= 1
                seq_iused = [[False for _ in range(len(seq_j) + 1)] for _ in range(len(seq_i) + 1)][i]
                aln_i.append(seq_iused)
        ...
        seq_i_aln = "".join(reversed(str(aln_i)))
        seq_j_aln = "".join(reversed(str(aln_j)))
...
alignments = print(we("FAT", "THEFASTCAT", -4, n_alignments=3, threshold=0))
